...
async def bootstrap(deps: RuntimeDeps) -> None:
    ...
    # NOTE: probe_map={} here is intentional — this discovery pass runs before
    # the OpenRouter adapter is created (that happens later in Step 5 "AI Routing").
    # Capabilities are re-probed via the HealthMonitor's probe_map after boot,
    # so an empty map here is safe and expected.
