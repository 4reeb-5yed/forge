...
- [x] 1. Docker & database foundation
  - [x] 1.1 Create Dockerfile for the backend
  ...
  - [x] 1.2 Create docker-compose.yml with postgres + forge-api services
    ...
    - _Requirements: Docker Setup_
  - [x] 1.3 Set up Alembic migrations scaffold
  ...
  - [x] 1.4 Create initial migration: sessions, audit_log, checkpoints, learning_outcomes
...
- [x] 2. Checkpoint — verify Docker + migrations work
  - Bring up `docker-compose up -d`, run `alembic upgrade head`, confirm tables exist. Ask the user if questions arise.

- [x] 3. OpenRouter AI Provider Adapter (Wave 1a)
  - [x] 3.1 Implement `app/adapters/openrouter.py`
...
- [x] 4. GitHub VCS Adapter (Wave 1b — parallel with 3)
  - [x] 4.1 Implement `app/adapters/github_vcs.py`
...
- [x] 5. Aider Coding Tool Adapter (Wave 1c — parallel with 3, 4)
  - [x] 5.1 Implement `app/adapters/aider_tool.py`
...
- [x] 6. Checkpoint — adapters complete
  - Ensure all adapter tests pass. Ask the user if questions arise.

- [x] 7. PostgreSQL persistence layer (Wave 2)
  - [x] 7.1 Create `app/db/pool.py` — connection pool setup
    ...
    - _Requirements: PostgreSQL Persistence_
  - [x] 7.2 Implement `app/db/session_store.py` — SessionManager backed by postgres
    ...
    - _Requirements: PostgreSQL Persistence_
  - [x] 7.3 Implement `app/db/audit_store.py` — AuditTrail backed by postgres
    ...
    - _Requirements: PostgreSQL Persistence_
  - [x] 7.4 Implement `app/db/checkpoint_store.py` — CrashRecovery checkpoint store
    ...
    - _Requirements: PostgreSQL Persistence_
  - [x] 7.5 Implement `app/db/learning_store.py` — LearningRecorder backed by postgres
...
- [x] 8. Checkpoint — persistence layer complete
  - Run migrations + integration tests against Docker postgres. Ensure all pass. Ask the user if questions arise.

- [x] 9. Integration wiring (Wave 3)
  - [x] 9.1 Update `app/workflow/bootstrap.py` to wire real adapters
